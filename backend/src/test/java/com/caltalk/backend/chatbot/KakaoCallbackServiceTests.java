...
class KakaoCallbackServiceTests {
    ...
    @SuppressWarnings("unchecked")
    @Test
    void retriesTemporaryCallbackFailureAndStopsAfterSuccess() throws Exception {
        KakaoScheduleAssistantService assistant = mock(KakaoScheduleAssistantService.class);
        HttpClient client = mock(HttpClient.class);
        ExecutorService executor = mock(ExecutorService.class);
        HttpResponse<String> unavailable = mock(HttpResponse.class);
        HttpResponse<String> success = mock(HttpResponse.class);
        when(assistant.reply("bot-user-1", "오늘 일정 알려줘")).thenReturn("등록된 일정이 없어요.");
