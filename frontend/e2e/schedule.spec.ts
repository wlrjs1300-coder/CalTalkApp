...
test('creates, reads, updates with location KEEP/SET/REMOVE, changes display timezone, and deletes', async ({
  ...
  try {
    ...
    await keepDialog.getByRole('button', { name: '일정 수정' }).click();
    ...
    await keepDialog.getByRole('button', { name: '변경사항 저장' }).click();
    ...
    await expect(page.getByRole('dialog')).toContainText('장소 미정');
    ...
    await page.getByRole('dialog').getByRole('button', { name: '일정 삭제' }).click();
    await page.getByRole('dialog').getByRole('button', { name: '일정 삭제' }).click();
